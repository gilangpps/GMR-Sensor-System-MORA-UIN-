...
# --- Konfigurasi Serial ---
PORT = '/dev/ttyUSB0'         # Ganti sesuai dengan port Arduino kamu
BAUD_RATE = 9600
    ...
    if collecting:
        try:
            if ser.in_waiting:
                ...
                while data_waktu and waktu - data_waktu[0] > WINDOW_DURATION:
                    data_waktu.pop(0)
                    data_tegangan.pop(0)

                # Update grafik
                line.set_data(data_waktu, data_tegangan)
                ax.set_xlim(waktu - WINDOW_DURATION, waktu)
                ax.relim()
