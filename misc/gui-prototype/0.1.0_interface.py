...
# --- Konfigurasi Serial ---
PORT = '/dev/ttyACM0'         # Ganti sesuai dengan port Arduino kamu
BAUD_RATE = 9600
...
def simpan_csv():
    ...
    if filepath:
        with open(filepath, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Waktu', 'Tegangan (V)'])
            for t, v in zip(data_waktu, data_tegangan):
                writer.writerow([t, v])
...
tk.Button(frame_btn, text="💾 Simpan CSV", command=simpan_csv, width=12).grid(row=0, column=3, padx=5)
